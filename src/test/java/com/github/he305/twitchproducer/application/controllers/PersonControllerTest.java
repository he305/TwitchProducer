...
class PersonControllerTest {
    ...
    @BeforeEach
    public void setUp() {
        underTest = new PersonController(personDao);
    }
    ...
    void getAllPersons() {
        ...
        Mockito.when(personDao.getAll()).thenReturn(personResponseDtoList);
    ...
    void getPersonById_existingEntry() {
        ...
        Mockito.when(personDao.getPersonById(Mockito.anyLong())).thenReturn(Optional.of(data));
    ...
    void getPersonByLastName_notExistingEntry() {
        PersonResponseDto data = new PersonResponseDto(0L, "test1", "test2", null);
        Mockito.when(personDao.getPersonById(Mockito.anyLong())).thenReturn(Optional.empty());
        ResponseEntity<PersonResponseDto> actual = underTest.getPersonById(0L);
    ...
    void addPerson_success() {
        ...
        Mockito.when(personDao.addPerson(Mockito.any())).thenReturn(expected);
    ...
    void addPerson_alreadyExist() {
        PersonAddDto data = new PersonAddDto("test1", "test2");
        Mockito.when(personDao.addPerson(Mockito.any())).thenThrow(EntityExistsException.class);
        ResponseEntity<PersonResponseDto> actual = underTest.addPerson(data);
    ...
    void deletePerson_notFound() {
        doThrow(new EntityNotFoundException()).when(personDao).deletePerson(Mockito.anyLong());
    ...
    void deletePerson_success() {
        doNothing().when(personDao).deletePerson(Mockito.anyLong());
        ResponseEntity<String> actual = underTest.deletePerson(0L);
    ...
    void updatePerson_notFound() {
        ...
        doThrow(new EntityNotFoundException()).when(personDao).updatePerson(0L, dto);
    ...
    void updatePerson_valid() {
        ...
        doReturn(expected).when(personDao).updatePerson(0L, dto);
        ResponseEntity<PersonResponseDto> actual = underTest.updatePerson(0L, dto);
        assertEquals(HttpStatus.OK, actual.getStatusCode());
        assertEquals(expected, actual.getBody());
    }
